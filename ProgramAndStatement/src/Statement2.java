...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        //creates this.rep as a Tree1
        this.rep = new Tree1<StatementLabel>();

        //creates root as StatementLabel
        StatementLabel root = new StatementLabel(Kind.BLOCK);

        //creates children as Sequence of Trees of StatementLabel
...
//        //temporary Sequence to hold children from disassemble
//        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
//
//        //call dissasemble into root to get root.kind
//        StatementLabel label = this.rep.disassemble(children);
//
//        //reassemble now that we have root
...
//        return label.kind;
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        //temporary Sequence to manipulate children after disassemble
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        //call disassemble to access children
        StatementLabel root = this.rep.disassemble(children);

        //create local instance of s as Statement2
        Statement2 localS = (Statement2) s;

        //add localS into children
        children.add(pos, localS.rep);

        //assemble this.rep with updated children
        this.rep.assemble(root, children);
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        //remove from children into s.rep
    ...
    public final int lengthOfBlock() {
        ...
        //simply return the number of Subtrees
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(children);
        localS1.rep = children.remove(0); // error with 1 and 0
        localS2.rep = children.remove(0); // or 0 or something
        this.createNewRep(); // clears this
        return label.condition;
    }
