...
public final class BinarySearchTreeMethods {
    ...
    public static <T extends Comparable<T>> boolean isInTree(BinaryTree<T> t,
        ...
        boolean checker = true; //boolean statement
        if (t.height() > 0) { //checking if tree is empty or not
            BinaryTree<T> left = t.newInstance();
            BinaryTree<T> right = t.newInstance();
            T rootNode = t.disassemble(left, right);
            if (rootNode.equals("")) {
                checker = false;
            } else if (rootNode.compareTo(x) < 0 && !checker) {
    ...
    public static <T> T removeSmallest(BinaryTree<T> t) {
        ...
        BinaryTree<T> right = t.newInstance();
        T rootNode = t.root();
        T rootNodeCopy = t.disassemble(left, right);

        if (left.size() > 0) { //use greater than 0 instead of does not equal

            rootNodeCopy = removeSmallest(left);
            t.assemble(rootNode, left, right);
