    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            ...
            case IF: {
                ...
                Statement.Condition condition = this.disassembleIf(subIf);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(condition) + " THEN");
                subIf.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(condition, subIf);
                break;
            }
            case IF_ELSE: {
                Statement subIf = this.newInstance();
                Statement subElse = this.newInstance();
                Statement.Condition condition = this.disassembleIfElse(subIf,
                        subElse);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(condition) + " THEN");
                subIf.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("ELSE");
                subElse.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(condition, subIf, subElse);
                break;
            }
            case WHILE: {
                Statement subWhile = this.newInstance();
                Statement.Condition condition = this.disassembleWhile(subWhile);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(condition) + " DO");
                subWhile.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(condition, subWhile);
                break;
            }
            case CALL: {
                String stringCall = this.disassembleCall();
                printSpaces(out, offset);
                out.println(stringCall);
                this.assembleCall(stringCall);
                break;
            }
            default: {
                // this will never happen...
